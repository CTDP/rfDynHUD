...
struct TelemWheel
{
    ...
    float mBrakeTemp;              // Kelvin
    float mPressure;               // kPa
    float mTemperature[3];         // Kelvin, left/center/right (not to be confused with inside/center/outside!)
};
...
struct TelemInfoBase
{
    ...
    float mEngineWaterTemp;        // Kelvin
    float mEngineOilTemp;          // Kelvin
...
// for noobs: TelemInfoV2 contains everything in TelemInfoBase, plus the following:
...
// may be expanded in the future to provide interfaces for drawing onscreen
struct GraphicsInfo
{
...
// for noobs: GraphicsInfoV2 contains everything in GraphicsInfo, plus the following:
...
// for noobs: VehicleScoringInfoV2 contains everything in VehicleScoringInfo, plus the following:
...
// for noobs: ScoringInfoV2 contains everything in ScoringInfoBase, plus the following:
...
// for noobs: InternalsPluginV2 contains everything in InternalsPlugin, plus the following:
...
// for noobs: InternalsPluginV3 contains everything in InternalsPluginV2 (and InternalsPlugin), plus the following:
