...
bool JVMTelemtryUpdateFunctions::init( JNIEnv* _env, jclass rfdynhudClass, jobject _rfdynhudObject )
{
    ...
    telemetryDataAddressKeeper = env->NewObject( DataAddressKeeper, DataAddressKeeper_constructor );
    scoringInfoAddressKeeper = env->NewObject( DataAddressKeeper2, DataAddressKeeper2_constructor );
    commentaryRequestInfoAddressKeeper = env->NewObject( DataAddressKeeper, DataAddressKeeper_constructor );
    graphicsInfoAddressKeeper = env->NewObject( DataAddressKeeper, DataAddressKeeper_constructor );
...
void JVMTelemtryUpdateFunctions::destroy()
{
    ...
    if ( telemetryDataAddressKeeper != NULL )
        env->DeleteLocalRef( telemetryDataAddressKeeper );
    if ( scoringInfoAddressKeeper != NULL )
        env->DeleteLocalRef( scoringInfoAddressKeeper );
    if ( graphicsInfoAddressKeeper != NULL )
        env->DeleteLocalRef( graphicsInfoAddressKeeper );
...
bool JVMConnection::init( const char* PLUGIN_PATH, const unsigned int resX, const unsigned int resY )
{
    ...
    rfDynHUD = env->CallStaticObjectMethod( RFDynHUD, mid, liveGameDataObjectsFactory, resX, resY );
