...
unsigned int cropLastPathComponent( char* path, const unsigned int length )
{
...
void cropLastPathComponent( char* path )
{
    cropLastPathComponent( path, strlen( path ) );
}
...
char* getRFactorPath()
{
    ...
    len = cropLastPathComponent( buffer, len ); // crop exe filename
    len = cropLastPathComponent( buffer, len ); // crop "Core" folder
